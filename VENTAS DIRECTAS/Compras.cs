...
namespace VENTAS_DIRECTAS
{
	...
	public partial class Compras : Form
	{
		...
		void llenartabla()
		{
			...
			try
			{
				comboBox2.Text = "Proveedores";
				comboBox2.Items.Clear();

				databaseConnection.Open();
				MySqlCommand command = new MySqlCommand("SELECT * FROM proveedor", databaseConnection);
				MySqlDataReader reader = command.ExecuteReader();
				while (reader.Read())
				{
					comboBox2.Refresh();
		...
		{
			string query = "UPDATE compras SET sucursal ='" + comboBox1.Text + "', proveedor = '" + comboBox2.Text + "', fecha = '" + dateTimePicker1.Text
				+ "' WHERE  codigo =" + textBox1.Text;
			databaseConnection.Open();
			MySqlCommand consulta = new MySqlCommand(query, databaseConnection);
			try
			{
				if (comboBox2.Text != "Proveedores" && comboBox1.Text != "Sucursales" )
				{
		...
		{
			string query = "INSERT INTO compras(sucursal, proveedor, fecha )"
				+" VALUES ('" + comboBox1.Text + "','" + comboBox2.Text + "','" + dateTimePicker1.Text + "')";
			databaseConnection.Open();
			MySqlCommand consulta = new MySqlCommand(query, databaseConnection);
			try
			{
		...
		{
			string query = "DELETE FROM compras WHERE  codigo =" + dataGridView1.CurrentRow.Cells[0].Value.ToString();
			databaseConnection.Open();
			MySqlCommand consulta = new MySqlCommand(query, databaseConnection);
			try
			{
